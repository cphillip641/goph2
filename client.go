...
// NewSftp returns new scp client and error if any.
func (c Client) NewScp() (scp.Client, error) {
...
// Upload a local file to remote server! via SCP
func (c Client) Upload(localPath string, remotePath string) (err error) {
	...
	err = scp.CopyFromFile(context.Background(), *local, remotePath, "0655")
...
func (c Client) Download(remotePath string, localPath string) (err error) {
	...
	err = scp.CopyFromRemote(context.Background(), local, remotePath)
...
func (c Client) UploadSftp(localPath string, remotePath string) (err error) {
...
func (c Client) DownloadSftp(remotePath string, localPath string) (err error) {
